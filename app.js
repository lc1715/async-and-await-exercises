...
async function drawCards() {
    try {
        let resp = await axios.get('https://www.deckofcardsapi.com/api/deck/new/shuffle/?deck_count=1')
        let deckId = resp.data.deck_id
        let cardResp1 = await axios.get(`https://www.deckofcardsapi.com/api/deck/${deckId}/draw/?count=1`)
        let cardResp2 = await axios.get(`https://www.deckofcardsapi.com/api/deck/${deckId}/draw/?count=1`)

        let { suit, value } = cardResp1.data.cards[0]
        console.log(`${value} of ${suit}`)

        let suit2 = cardResp2.data.cards[0].suit
        let value2 = cardResp2.data.cards[0].value
